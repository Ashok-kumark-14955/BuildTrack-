...
const rows = db_1.default
    .prepare(`SELECT d.id, d.name, d.columnPositions FROM drawings d
     JOIN projects p ON p.id = d.projectId
     WHERE p.code = 'ASIC-P1'`)
    .all();
const update = db_1.default.prepare('UPDATE drawings SET columnPositions = ? WHERE id = ?');
for (const row of rows) {
    ...
    if (Object.keys(existing).length >= COLS.length * ROWS.length)
        continue;
