    ...
    try {
        ...
        if (!drawingsTableInfo) {
            return res.json({
                ...
                tablesFound: tablesRaw.map((t) => t.table_name),
            });
        }
...
app.post('/api/debug-update-drawing', async (req, res) => {
    try {
        ...
        if (!drawingId || !col)
            return res.status(400).json({ error: 'drawingId and col required' });
        ...
        if (!rowId)
            return res.json({ ok: false, error: 'ROWID not found', selectResult });
        // 2. Try SDK updateRow
        ...
        let sdkResult;
        try {
            sdkResult = await table.updateRow(updatePayload);
...
async function runMigrations() {
    ...
    try {
        ...
        const tableListResult = await new Promise((resolve, reject) => {
        ...
        const tables = tableListResult?.data || [];
        const drawingsTable = tables.find((t) => (t.table_name || '').toLowerCase() === 'drawings');
        if (!drawingsTable) {
        ...
        const colListResult = await new Promise((resolve, reject) => {
            const req = https.request({
                hostname: 'api.catalyst.zoho.in',
        ...
        const existingCols = (colListResult?.data || []).map((c) => (c.column_name || '').toLowerCase());
...
function makeCatalystManagementApi(req) {
    ...
    function catalystRequest(method, apiPath, body, overrideToken) {
