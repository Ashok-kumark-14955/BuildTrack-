...
function writeSvg(svg) {
    ...
    fs_1.default.writeFileSync(path_1.default.join(uploadDir, fileName), svg);
...
function paintingSvg() {
    ...
    const shapes = rooms
        .map((r) => `<rect x="${r.x}" y="${r.y}" width="${r.w}" height="${r.h}" fill="${r.color}" stroke="#555" stroke-width="3"/>` +
...
function columnGridSvg() {
    ...
    for (let r = 0; r < rowNums.length; r++) {
        for (let c = 0; c < cols.length; c++) {
            ...
            columns.push(`<rect x="${x - 18}" y="${y - 18}" width="36" height="36" fill="#9ca3af" stroke="#111" stroke-width="3"/>` +
...
db_1.default.prepare('INSERT INTO projects (id, name, createdAt) VALUES (?, ?, ?)').run(projectId, 'Riverside Tower', now);
const insertTask = db_1.default.prepare(`INSERT INTO tasks (id, drawingId, gridCode, name, description, category, priority, assignedTo, startDate, dueDate, status, progress, createdAt, updatedAt)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
...
function seedDrawing(opts) {
    ...
    db_1.default.prepare(`INSERT INTO drawings (id, projectId, name, fileUrl, fileType, gridCols, gridRows, createdAt)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(drawingId, projectId, opts.name, opts.fileUrl, 'image', opts.gridCols, opts.gridRows, now);
    opts.cells.forEach(([col, row], i) => {
        ...
        insertTask.run((0, uuid_1.v4)(), drawingId, code, `${category} work - ${code}`, `Sample task for grid ${code} demonstrating ${status} status.`, category, priorities[i % priorities.length], engineers[i % engineers.length], '2026-07-01', '2026-08-15', status, progress, now, now);
    });
    insertActivity.run((0, uuid_1.v4)(), null, drawingId, `${opts.name} seeded with demo tasks`, now);
    return drawingId;
}
