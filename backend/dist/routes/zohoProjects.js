...
function getPortalId() {
    ...
    if (!pid)
        throw new Error('ZOHO_PORTAL_ID env var is required');
    return pid;
}
// ─── Token exchange ────────────────────────────────────────────────────────────
...
function normalizeTask(zt) {
    ...
    return {
        ...
        assignedTo: (zt.details?.owners || []).map((o) => o.name).join(', ') || '',
...
router.get('/projects/:zpId', handle(async (req, res) => {
    ...
    const data = await zohoGet(token, `/portal/${portalId}/projects/`);
    const raw = (data.projects || []).find((p) => String(p.id_string) === req.params.zpId);
    if (!raw)
...
router.post('/projects', handle(async (req, res) => {
    const token = await (0, zohoAuth_1.getAccessToken)();
    const portalId = getPortalId();
    const { name, description, startDate, endDate, managerName } = req.body;
    if (!name?.trim())
        return void res.status(400).json({ error: 'name is required' });
    const formBody = { name: name.trim() };
    if (description)
        formBody.description = description;
    if (startDate)
        formBody.start_date = startDate;
    if (endDate)
        formBody.end_date = endDate;
    if (managerName)
        formBody.owner = managerName;
    const data = await zohoPostForm(token, `/portal/${portalId}/projects/`, formBody);
    const created = (data.projects || [])[0];
    if (!created)
        return void res.status(500).json({ error: 'Zoho did not return the created project', raw: data });
...
router.put('/projects/:zpId', handle(async (req, res) => {
    ...
    if (!updated)
        return void res.status(500).json({ error: 'Zoho did not return the updated project', raw: data });
...
router.get('/projects/:zpId/milestones', handle(async (req, res) => {
    ...
    const milestones = (data.milestones || []).map((m) => normalizeMilestone({ ...m, project_id: req.params.zpId }));
    res.json(milestones);
}));
...
router.post('/projects/:zpId/milestones', handle(async (req, res) => {
    ...
    if (!created)
        return void res.status(500).json({ error: 'Zoho did not return the created milestone', raw: data });
...
router.put('/projects/:zpId/milestones/:milestoneId', handle(async (req, res) => {
    ...
    if (dueDate)
        formBody.end_date = dueDate;
    if (status)
        formBody.flag = status === 'Completed' ? 'closed' : 'open';
    const data = await zohoPutForm(token, `/portal/${portalId}/projects/${req.params.zpId}/milestones/${req.params.milestoneId}/`, formBody);
    const updated = (data.milestones || [])[0];
    if (!updated)
        return void res.status(500).json({ error: 'Zoho did not return updated milestone', raw: data });
...
router.delete('/projects/:zpId/milestones/:milestoneId', handle(async (req, res) => {
    const token = await (0, zohoAuth_1.getAccessToken)();
    const portalId = getPortalId();
    const data = await zohoDelete(token, `/portal/${portalId}/projects/${req.params.zpId}/milestones/${req.params.milestoneId}/`);
    res.json(data);
}));
...
router.get('/projects/:zpId/tasks', handle(async (req, res) => {
    ...
    const allTasks = [];
    while (true) {
        const data = await zohoGet(token, `/portal/${portalId}/projects/${req.params.zpId}/tasks/?range=${index},100`);
...
router.post('/projects/:zpId/tasks', handle(async (req, res) => {
    const token = await (0, zohoAuth_1.getAccessToken)();
    const portalId = getPortalId();
    const { name, description, dueDate, priority, status, tasklistId } = req.body;
    if (!name?.trim())
        return void res.status(400).json({ error: 'name is required' });
    const formBody = { name: name.trim() };
    if (description)
    ...
    if (dueDate)
        formBody.end_date = dueDate;
    if (priority)
        formBody.priority = priority.toLowerCase();
    if (status)
        formBody.status = status;
    if (tasklistId)
        formBody.tasklist_id = tasklistId;
    const data = await zohoPostForm(token, `/portal/${portalId}/projects/${req.params.zpId}/tasks/`, formBody);
    const created = (data.tasks || [])[0];
    if (!created)
        return void res.status(500).json({ error: 'Zoho did not return the created task', raw: data });
...
router.put('/projects/:zpId/tasks/:taskId', handle(async (req, res) => {
    const token = await (0, zohoAuth_1.getAccessToken)();
    const portalId = getPortalId();
    const { name, description, dueDate, priority, status, progress } = req.body;
    const formBody = {};
    if (name)
        formBody.name = name;
    if (description)
    ...
    if (progress)
        formBody.percent_complete = progress;
    const data = await zohoPutForm(token, `/portal/${portalId}/projects/${req.params.zpId}/tasks/${req.params.taskId}/`, formBody);
    const updated = (data.tasks || [])[0];
    if (!updated)
        return void res.status(500).json({ error: 'Zoho did not return the updated task', raw: data });
...
router.delete('/projects/:zpId/tasks/:taskId', handle(async (req, res) => {
    const token = await (0, zohoAuth_1.getAccessToken)();
    const portalId = getPortalId();
    const data = await zohoDelete(token, `/portal/${portalId}/projects/${req.params.zpId}/tasks/${req.params.taskId}/`);
    res.json(data);
}));
// ═══════════════════════════════════════════════════════════════════════════════
...
router.post('/seed', handle(async (_req, res) => {
    ...
    if (!project)
        return void res.status(500).json({ error: 'Failed to create project', raw: projData });
    ...
    for (const t of tasks) {
        ...
        if (t.start)
            formBody.start_date = t.start;
        if (t.end)
            formBody.end_date = t.end;
        if (t.status !== 'Open')
            formBody.status = t.status;
