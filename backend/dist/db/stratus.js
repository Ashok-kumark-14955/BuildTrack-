...
exports.isStratusEnabled = isStratusEnabled;
 ...
 */
function isStratusEnabled() {
    const enabled = Boolean(process.env.X_ZOHO_CATALYST_LISTEN_PORT ||
    ...
    if (!_stratusLogged) {
        _stratusLogged = true;
        const keys = Object.keys(process.env).filter((k) => k.includes('CATALYST') || k.includes('ZOHO') || k === 'USE_STRATUS');
