...
function foundationPlanSvg() {
  ...
  <!-- Column footings -->
  ${[100,760,1420].flatMap(x => [200,700].map(y =>
    `<circle cx="${x+40}" cy="${y+40}" r="30" fill="#c9a87c" stroke="#5c4324" stroke-width="3"/>`
  )).join('')}
  <!-- Grid labels -->
...
function electricalPlanSvg() {
  ...
  const circles = symbols.map(s =>
    ...
    `<text x="${s.x}" y="${s.y+6}" font-size="16" fill="#92400e" font-family="sans-serif" text-anchor="middle" font-weight="bold">${s.label}</text>`
  ).join('');
  // wiring lines horizontal per row
  const wires = [200,500,800].map(y =>
    `<line x1="150" y1="${y}" x2="1400" y2="${y}" stroke="#ca8a04" stroke-width="2"/>`
  ).join('');
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="1000" viewBox="0 0 1600 1000">
...
function plumbingPlanSvg() {
  ...
  ${[
    ...
    `<text x="${f.x}" y="${f.y+6}" font-size="14" fill="#1e3a8a" font-family="sans-serif" text-anchor="middle" font-weight="bold">${f.label}</text>`
  ).join('')}
...
export async function seedHouseProject(req: Request): Promise<void> {
  ...
  await db.run(req,
    `INSERT INTO projects (id, name, code, description, startDate, endDate, status, managerName, archived, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [projectId, PROJECT_NAME, 'HBP-001', 'A comprehensive residential house building project covering all construction phases from foundation to finishing.', '2026-01-01', '2026-12-31', 'Active', 'Rajesh Sharma', false, now, now]
  );
  ...
  for (const ms of milestoneData) {
    ...
    msIds.push(id);
    await db.run(req,
      `INSERT INTO milestones (id, projectId, name, description, status, dueDate, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, projectId, ms.name, ms.description, ms.status, ms.dueDate, now, now]
    );
  }
  ...
  async function seedDrawing(opts: {
    ...
    await db.run(req,
      `INSERT INTO drawings (id, projectId, milestoneId, name, fileUrl, fileType, gridCols, gridRows, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [drawingId, projectId, msIds[opts.milestoneIndex], opts.name, fileUrl, 'image', opts.gridCols, opts.gridRows, now]
    );
    ...
    for (const t of opts.tasks) {
      ...
      await db.run(req,
        `INSERT INTO tasks (id, drawingId, gridCode, elementId, elementType, name, description, category, priority, assignedTo, startDate, dueDate, status, progress, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [uuid(), drawingId, gridCode, elementId, 'column', t.name, t.description, t.category, t.priority, t.assignedTo, '2026-01-01', t.dueDate, t.status, t.progress, now, now]
      );
    }

    await db.run(req,
      `INSERT INTO activity (id, taskId, drawingId, message, createdAt) VALUES (?, ?, ?, ?, ?)`,
      [uuid(), null, drawingId, `${opts.name} added to House Building Project`, now]
    );
  }
  ...
  await db.run(req,
    `INSERT INTO activity (id, taskId, drawingId, message, createdAt) VALUES (?, ?, ?, ?, ?)`,
    [uuid(), null, null, 'House Building Project seeded with 6 drawings, 5 milestones and 53 tasks', now]
  );
