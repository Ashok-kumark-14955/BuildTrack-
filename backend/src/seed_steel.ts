...
function foundationPlanSvg(): string {
  ...
  const vLines = cols.map((_, i) =>
    `<line x1="${originX + i * spacingX}" y1="${originY - 50}"
           x2="${originX + i * spacingX}" y2="${originY + (rows.length - 1) * spacingY + 50}"
           stroke="#8B6914" stroke-width="1.5" stroke-dasharray="8 5"/>`
  ).join('');
  const hLines = rows.map((_, i) =>
    `<line x1="${originX - 50}" y1="${originY + i * spacingY}"
  ...
  const colLabels = cols.map((c, i) =>
    `<circle cx="${originX + i * spacingX}" cy="${originY - 70}" r="18" fill="#fff" stroke="#5c3d11" stroke-width="2"/>
     <text x="${originX + i * spacingX}" y="${originY - 64}" font-size="16" fill="#5c3d11" font-family="sans-serif" text-anchor="middle" font-weight="bold">${c}</text>`
  ).join('');
  const rowLabels = rows.map((n, i) =>
    `<circle cx="${originX - 70}" cy="${originY + i * spacingY}" r="18" fill="#fff" stroke="#5c3d11" stroke-width="2"/>
     <text x="${originX - 70}" y="${originY + i * spacingY + 6}" font-size="16" fill="#5c3d11" font-family="sans-serif" text-anchor="middle" font-weight="bold">${n}</text>`
  ).join('');

  // Footing pads (large square + inner square)
  const footings: string[] = [];
  const pilings: string[] = [];
  for (let r = 0; r < rows.length; r++) {
    for (let c = 0; c < cols.length; c++) {
      const x = originX + c * spacingX;
      const y = originY + r * spacingY;
      footings.push(
        `<rect x="${x - 30}" y="${y - 30}" width="60" height="60" fill="#d4b483" stroke="#5c3d11" stroke-width="2.5"/>
  ...
  // Grade beams
  const gradeBeams = rows.map((_, i) =>
    `<line x1="${originX - 30}" y1="${originY + i * spacingY}"
           x2="${originX + (cols.length - 1) * spacingX + 30}" y2="${originY + i * spacingY}"
           stroke="#8B6914" stroke-width="8" stroke-linecap="round"/>`
  ).join('') + cols.map((_, i) =>
    `<line x1="${originX + i * spacingX}" y1="${originY - 30}"
...
function columnErectionSvg(): string {
  const cols = ['A', 'B', 'C', 'D', 'E', 'F'];
  const rows = [1, 2, 3, 4, 5];
  const originX = 200, originY = 190, spacingX = 220, spacingY = 140;

  const vLines = cols.map((_, i) =>
    `<line x1="${originX + i * spacingX}" y1="${originY - 60}"
           x2="${originX + i * spacingX}" y2="${originY + (rows.length - 1) * spacingY + 60}"
           stroke="#aab4be" stroke-width="1" stroke-dasharray="8 5"/>`
  ).join('');
  const hLines = rows.map((_, i) =>
    `<line x1="${originX - 60}" y1="${originY + i * spacingY}"
           x2="${originX + (cols.length - 1) * spacingX + 60}" y2="${originY + i * spacingY}"
           stroke="#aab4be" stroke-width="1" stroke-dasharray="8 5"/>`
  ).join('');

  const colLabels = cols.map((c, i) =>
    `<circle cx="${originX + i * spacingX}" cy="${originY - 80}" r="18" fill="#fff" stroke="#1e3a5f" stroke-width="2"/>
     <text x="${originX + i * spacingX}" y="${originY - 74}" font-size="16" fill="#1e3a5f" font-family="sans-serif" text-anchor="middle" font-weight="bold">${c}</text>`
  ).join('');
  const rowLabels = rows.map((n, i) =>
    `<circle cx="${originX - 80}" cy="${originY + i * spacingY}" r="18" fill="#fff" stroke="#1e3a5f" stroke-width="2"/>
     <text x="${originX - 80}" y="${originY + i * spacingY + 6}" font-size="16" fill="#1e3a5f" font-family="sans-serif" text-anchor="middle" font-weight="bold">${n}</text>`
  ).join('');
  ...
  for (let r = 0; r < rows.length; r++) {
    for (let c = 0; c < cols.length; c++) {
      ...
      columns.push(
        `<rect x="${x - 14}" y="${y - 14}" width="28" height="4" fill="#1e3a5f"/>
  ...
  for (let r = 0; r < rows.length; r++) {
    for (let c = 0; c < cols.length; c++) {
      ...
      const y = originY + r * spacingY;
      basePlates.push(
        `<circle cx="${x}" cy="${y}" r="22" fill="none" stroke="#dc2626" stroke-width="1.5" stroke-dasharray="5 4"/>`
      );
    }
  }
  ...
  const levelMarks = levels.map((lv, i) =>
    `<line x1="${originX + (cols.length - 1) * spacingX + 70}" y1="${originY + i * spacingY}"
           x2="${originX + (cols.length - 1) * spacingX + 140}" y2="${originY + i * spacingY}"
           stroke="#dc2626" stroke-width="1.5"/>
     <text x="${originX + (cols.length - 1) * spacingX + 145}" y="${originY + i * spacingY + 5}" font-size="12" fill="#dc2626" font-family="sans-serif">${lv}</text>`
  ).join('');
...
function beamErectionSvg(): string {
  const cols = ['A', 'B', 'C', 'D', 'E', 'F'];
  const rows = [1, 2, 3, 4, 5];
  const originX = 200, originY = 190, spacingX = 220, spacingY = 140;
  ...
  const colLabels = cols.map((c, i) =>
    `<circle cx="${originX + i * spacingX}" cy="${originY - 80}" r="18" fill="#fff" stroke="#065f46" stroke-width="2"/>
     <text x="${originX + i * spacingX}" y="${originY - 74}" font-size="16" fill="#065f46" font-family="sans-serif" text-anchor="middle" font-weight="bold">${c}</text>`
  ).join('');
  const rowLabels = rows.map((n, i) =>
    `<circle cx="${originX - 80}" cy="${originY + i * spacingY}" r="18" fill="#fff" stroke="#065f46" stroke-width="2"/>
     <text x="${originX - 80}" y="${originY + i * spacingY + 6}" font-size="16" fill="#065f46" font-family="sans-serif" text-anchor="middle" font-weight="bold">${n}</text>`
  ).join('');

  // Column stubs (small squares)
  const columnStubs: string[] = [];
  for (let r = 0; r < rows.length; r++) {
    for (let c = 0; c < cols.length; c++) {
      const x = originX + c * spacingX;
      const y = originY + r * spacingY;
      columnStubs.push(
        `<rect x="${x - 8}" y="${y - 8}" width="16" height="16" fill="#9ca3af" stroke="#1e3a5f" stroke-width="2"/>`
      );
    }
  }

  // Primary beams (horizontal, bold blue line + section label)
  const primaryBeams: string[] = [];
  for (let r = 0; r < rows.length; r++) {
    for (let c = 0; c < cols.length - 1; c++) {
      ...
      primaryBeams.push(
        `<line x1="${x1}" y1="${y}" x2="${x2}" y2="${y}" stroke="#065f46" stroke-width="6" stroke-linecap="round"/>
  ...
  for (let r = 0; r < rows.length - 1; r++) {
    for (let c = 0; c < cols.length; c++) {
      ...
      secondaryBeams.push(
        `<line x1="${x}" y1="${y1}" x2="${x}" y2="${y2}" stroke="#0284c7" stroke-width="3.5" stroke-linecap="round"/>
  ...
  for (let r = 0; r < rows.length; r++) {
    for (let c = 0; c < cols.length; c++) {
      ...
      const y = originY + r * spacingY;
      connections.push(
        `<circle cx="${x}" cy="${y}" r="10" fill="none" stroke="#dc2626" stroke-width="1.5"/>`
      );
    }
  }
...
function rafterErectionSvg(): string {
  const cols = ['A', 'B', 'C', 'D', 'E', 'F'];
  const rows = [1, 2, 3, 4, 5];
  ...
  for (let c = 0; c < cols.length; c++) {
    for (let r = 0; r < rows.length; r++) {
      ...
      rafters.push(
        `<line x1="${x}" y1="${y}" x2="${x}" y2="${ridgeY + 15}" stroke="#a855f7" stroke-width="4" stroke-linecap="round"/>
  ...
  for (let p = 1; p < 5; p++) {
    ...
    purlins.push(
      `<line x1="${originX}" y1="${py}" x2="${originX + (cols.length - 1) * spacingX}" y2="${py}"
  ...
  // Bracing diagonals
  const bracings: string[] = [];
  for (let r = 0; r < rows.length - 1; r++) {
    const y1 = originY + r * spacingY;
    const y2 = originY + (r + 1) * spacingY;
    bracings.push(
      `<line x1="${originX}" y1="${y1}" x2="${originX + spacingX}" y2="${y2}" stroke="#ef4444" stroke-width="1.5" stroke-dasharray="6 4"/>`,
      `<line x1="${originX + spacingX}" y1="${y1}" x2="${originX}" y2="${y2}" stroke="#ef4444" stroke-width="1.5" stroke-dasharray="6 4"/>`
    );
  }

  // Grid labels
  const colLabels = cols.map((c, i) =>
    `<circle cx="${originX + i * spacingX}" cy="${originY + (rows.length - 1) * spacingY + 60}" r="18" fill="#fff" stroke="#7c3aed" stroke-width="2"/>
     <text x="${originX + i * spacingX}" y="${originY + (rows.length - 1) * spacingY + 66}" font-size="16" fill="#7c3aed" font-family="sans-serif" text-anchor="middle" font-weight="bold">${c}</text>`
  ).join('');
  const rowLabels = rows.map((n, i) =>
    `<circle cx="${originX - 80}" cy="${originY + i * spacingY}" r="18" fill="#fff" stroke="#7c3aed" stroke-width="2"/>
     <text x="${originX - 80}" y="${originY + i * spacingY + 6}" font-size="16" fill="#7c3aed" font-family="sans-serif" text-anchor="middle" font-weight="bold">${n}</text>`
  ).join('');
...
interface TaskDef {
  ...
  category: string; priority: string; engineer: string;
  start: string; due: string; status: string; progress: number;
  elementType: string; elementId: string; milestoneId: string;
}

const roofSheetTaskDefs = (milestoneId: string): TaskDef[] => [
...
function insertRoofSheetDrawing(projectId: string, milestoneId: string, projectNow: string): string {
  ...
  localDb.prepare(
    `INSERT INTO drawings (id, projectId, milestoneId, name, fileUrl, fileType, gridCols, gridRows, columnPositions, createdAt)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(roofSheetDrawingId, projectId, milestoneId,
    'STR-RSL-005 Roof Sheet Layout',
    writeSvgDataUrl(roofSheetLayoutSvg()),
    'image', 6, 5,
...
async function ensureRoofSheetDrawing(req: Request, projectId: string): Promise<void> {
  const drawing = await db.get(req, `SELECT id FROM drawings WHERE projectId = ? AND name = ?`, [projectId, 'STR-RSL-005 Roof Sheet Layout']);
  if (drawing) return;
  const milestone: any = await db.get(req, `SELECT id FROM milestones WHERE projectId = ? AND name = ?`, [projectId, 'M5 – Roof Rafter & Cladding']);
  if (!milestone) return;
...
export async function seedSteelProject(req: Request): Promise<void> {
  const existing: any = await db.get(req, `SELECT id FROM projects WHERE name = ?`, [PROJECT_NAME]);
  if (existing) {
...
localDb.prepare(
  ...
  projectId,
  'Apex Steel Industrial Complex – Phase 1',
  'ASIC-P1',
  'Full steel structural building comprising basement foundation, steel column erection, primary/secondary beam erection, and roof rafter erection. Six-bay × five-bay grid, four storeys + roof.',
  '2026-03-01',
  '2026-12-31',
  'In Progress',
  'Rajesh Nair (Sr. Project Manager)',
  projectNow,
  projectNow
);
...
insertMilestone.run(milestoneIds.m1, projectId,
  'M1 – Site Preparation & Earthworks',
  'Topsoil removal, excavation to formation level, dewatering, and setting out of grid lines.',
  '2026-04-15', 'Completed', projectNow, projectNow);

insertMilestone.run(milestoneIds.m2, projectId,
  'M2 – Piling & Foundation',
  'Bored pile installation (600 dia), pile caps, grade beams, and basement slab concrete works.',
  '2026-06-30', 'Active', projectNow, projectNow);

insertMilestone.run(milestoneIds.m3, projectId,
  'M3 – Steel Column Erection',
  'Delivery, fabrication inspection, crane lifting, plumbing, and final bolting of all UC columns.',
  '2026-08-15', 'Active', projectNow, projectNow);

insertMilestone.run(milestoneIds.m4, projectId,
  'M4 – Steel Beam & Decking',
  'Primary/secondary UB beam erection, shear stud installation, metal deck placement and composite slab.',
  '2026-10-31', 'Pending', projectNow, projectNow);

insertMilestone.run(milestoneIds.m5, projectId,
  'M5 – Roof Rafter & Cladding',
  'Ridge beam, rafter, purlin, bracing erection; roof cladding, gutters, and weatherproofing.',
...
for (const t of foundationTasks) {
  ...
  insertActivity.run(uuid(), taskId, foundDrawingId, `Task "${t.name}" created – status: ${t.status}`, projectNow);
  if (t.status === 'Completed') {
    insertActivity.run(uuid(), taskId, foundDrawingId, `Task "${t.name}" marked Completed by ${t.engineer}`, projectNow);
  }
}
insertActivity.run(uuid(), null, foundDrawingId, 'STR-FND-001 Basement Foundation Plan uploaded and tasks assigned', projectNow);
...
for (const t of columnTasks) {
  ...
  insertActivity.run(uuid(), taskId, colDrawingId, `Task "${t.name}" created – status: ${t.status}`, projectNow);
  if (t.status === 'Completed') {
    insertActivity.run(uuid(), taskId, colDrawingId, `Task "${t.name}" marked Completed by ${t.engineer}`, projectNow);
  }
}
insertActivity.run(uuid(), null, colDrawingId, 'STR-COL-002 Steel Column Erection Plan uploaded and tasks assigned', projectNow);
...
for (const t of beamTasks) {
  ...
  insertActivity.run(uuid(), taskId, beamDrawingId, `Task "${t.name}" created – status: ${t.status}`, projectNow);
}
insertActivity.run(uuid(), null, beamDrawingId, 'STR-BEA-003 Steel Beam Erection Plan uploaded and tasks assigned', projectNow);
...
for (const t of rafterTasks) {
  ...
  insertActivity.run(uuid(), taskId, rafterDrawingId, `Task "${t.name}" created – status: ${t.status}`, projectNow);
}
...
for (const pt of projectLevelTasks) {
  insertProjectTask.run(
    uuid(), projectId, pt.name, pt.desc, pt.priority, pt.status,
    pt.assignee, pt.due, pt.hours, pt.tags, projectNow, projectNow
  );
}
