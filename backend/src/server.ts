...
app.post('/api/migrate/add-sortorder-column', async (req, res) => {
  try {
    ...
    const candidates = [process.env.CATALYST_AUTH_TOKEN, sdkToken, zohoProjectsToken].filter(Boolean) as string[];
    ...
    for (const candidate of candidates) {
      ...
      const asArray: any[] = Array.isArray(tableListRaw) ? tableListRaw : (Array.isArray(tableListRaw?.data) ? tableListRaw.data : []);
      if (asArray.length > 0) {
    ...
    const drawingsTableInfo = tablesRaw.find((t: any) => (t.table_name || '').toLowerCase() === 'drawings');
    if (!drawingsTableInfo) {
      return res.json({ ok: false, error: 'drawings table not found', tablesFound: tablesRaw.map((t: any) => t.table_name) });
    }
    const tableId = String(drawingsTableInfo.table_id);
